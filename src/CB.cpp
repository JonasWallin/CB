...
Rcpp::List    c_basis1(Eigen::MappedSparseMatrix<double> A) {
  ...
  Eigen::SparseMatrix<double>  PA = A *P;
  Eigen::JacobiSVD<Eigen::MatrixXd> svd( Eigen::MatrixXd(PA.leftCols(counter)), Eigen::ComputeFullV | Eigen::ComputeFullU );
  ...
  for(int i=0; i< counter; i++){
    ...
    for(int j=0; j< counter; j++){
      tripletList.push_back(Eigen::Triplet<double>(i_i, index[j], V(i,j)));
    }
  }
